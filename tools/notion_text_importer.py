...
transformed_data = {'_data':{row['key']: {"it": row['it'], "en": row['en']} for _, row in dfText.iterrows()}}
json_result = json.dumps(transformed_data, indent=4, ensure_ascii=False)


with open('Assets/DialogueSystem/Resources/texts.json', 'w', encoding='utf8') as f:
    f.write(json_result)

dfdialog = n2p.from_notion_DB_to_dataframe(database_dialogs_id)

dfdialog['testi'] = dfdialog['testi'].apply(ast.literal_eval)
transformed_data = {'_data':{row['key']: row['testi'] for _, row in dfdialog.iterrows()}}
json_result = json.dumps(transformed_data, indent=4, ensure_ascii=False)

with open('Assets/DialogueSystem/Resources/dialogues.json', 'w', encoding='utf8') as f:
    f.write(json_result)